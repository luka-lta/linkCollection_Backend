...
class LinkObjectRepository
{
    ...
    public function deleteLink(int $linkId): void
    {
        ...
        $sql = <<<SQL
            DELETE FROM link_list WHERE link_id = :linkId
        SQL;
        try {
            $statement = $this->database->prepare($sql);
            $statement->execute([
    ...
    public function updateLink(LinkObject $linkObject): void
    {
        ...
        $sql = <<<SQL
            UPDATE link_list SET name = :name, url = :url, displayName = :displayname WHERE link_id = :linkId
        SQL;
        try {
            $statement = $this->database->prepare($sql);
            $statement->execute([
    ...
    {
        $sql = <<<SQL
            SELECT * FROM link_list WHERE page_id = :pageId
        SQL;
        try {
            $statement = $this->database->prepare($sql);
            $statement->execute([
    ...
    {
        $sql = <<<SQL
            SELECT * FROM link_list WHERE link_id = :linkId
        SQL;
        $statement = $this->database->prepare($sql);
        $statement->execute([
            'linkId' => $linkId
